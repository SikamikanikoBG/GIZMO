        ...
        with open(os.path.join(params_path + definitions.selected_param_file), 'r', encoding='utf-8') as param_file:
            ...
            with st.form("Settings"):
                ...
                with col1:
                    ...
                    new_value_criterion_column = st.text_input(label=f"{'criterion_column'}.", value=json_object['criterion_column'])
                    new_value_custom_calculations = st.text_input(label=f"{'custom_calculations'}",value=json_object['custom_calculations'])


                    new_value_main_table = st.text_input(label=f"{'main_table'}. Well, the name of the main data file for the project.", value=json_object['main_table'])
                    new_value_additional_tables = st.text_input(label=f"{'additional_tables'}. Jizzmo will left join them to the main table based on keys that you are specifying here.",
                    ...
                    new_value_periods_to_exclude = st.text_input(label=f"{'periods_to_exclude'}. Example - no full performance period, or bad nb of cases etc.",
                ...
                if submitted:
                    with open(os.path.join(params_path + selected_param_file), 'w', encoding='utf-8') as output_param_file:
