...
nice -n 16 python3 -W ignore $DIR/load_data.py --session gridsearch --volumes 25000
...
cat $DIR/run_simulation_parallel_0|/home/ardi/apps/parallel/bin/parallel --progress --bar --load 80% --memfree 1G --colsep ' ' -j 0 nice -n 16 python3 -W ignore $DIR/grid_search.py  --project {1} --tp {2} --sl {3} --training_rows {4} --period {5} --nb_features 50
curl https://notify.run/QcLwLsIj07xKPxH81Eys -d "Simu priority 0 are ready! Go and check them on www.snastroenie.com!"
